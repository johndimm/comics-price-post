...
import { getComicMetadata } from "@/lib/db";

export function comicEra(year: number): string {
...
export function ebayTitle(comic: Comic, gradeLabel: string): string {
...
export function conditionFromGrade(grade: number): string {
...
export function buildDescription(comic: Comic, gradeLabel: string, metadata: ReturnType<typeof getComicMetadata>, comicId: string): string {
    const era = comicEra(comic.year);
    ...
    if (isSlabbed) {
        ...
        const communityHigh = parseFloat(String(comic.community_high ?? "0")) || 0;
        const communityMismatch = communityHigh > 0 && (comic.grade - communityHigh) > 1.0;
        if (comic.community_url && !communityMismatch) {
